...
def _send_whatsapp(receiver: str, message: str) -> str:
    ...
    if is_phone:
        # Use WhatsApp Web direct API link - goes straight to the contact's conversation
        import subprocess
        url = f"https://web.whatsapp.com/send?phone={clean_num}&text="
        ...
        # Wait for WhatsApp Web to load and the chat to open
        time.sleep(6.0)
        
        # Type the message in the input field and send
        _paste_text(message)
        time.sleep(0.3)
        pyautogui.press("enter")
        time.sleep(0.5)
        return f"Message sent to {receiver} via WhatsApp Web (direct link)."
    else:
        # For contact names, fall back to desktop app search
