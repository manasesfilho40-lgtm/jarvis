...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not win:
            print("[GameUpdater] [!] Steam penceresi bulunamadi")
            return False
        ...
        if not colorful.any():
            print("[GameUpdater] [!] Avatar rengi bulunamadi - tahminle tiklanıyor")
        ...
        print(f"[GameUpdater] [*] Profil avatarı ({abs_x}, {abs_y}) - tiklanıyor")
        pyautogui.click(abs_x, abs_y)
        return True

    except ImportError as e:
        print(f"[GameUpdater] [!] Eksik kutuphane: {e}")
        return False
    except Exception as e:
        print(f"[GameUpdater] [!] Profil tespiti basarisiz: {e}")
        return False
...
def _handle_steam_profile_selection() -> bool:
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            print("[GameUpdater] [i] Profil dialogu yok - Steam zaten giris yapmis")
            return False
    except ImportError:
        pass
    except Exception:
    ...
    print("[GameUpdater] [*] Profil secimi tespit edildi - ilk profile tiklanıyor")
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    try:
        ...
        print(f"[GameUpdater] [!] pywinauto basarisiz: {e}")
        return _handle_install_dialog_pyautogui(game_name, best_drive)
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    print("[GameUpdater] [!] Steam baslatilamadi")
    return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        print(f"[GameUpdater] [!] AppID arama basarisiz: {e}")
